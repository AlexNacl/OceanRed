...
public class PlayerMovementHuman : MonoBehaviour
{
	...
	void GroundMovement()
	{
		...
		if (input.lookUp && !isCrouching && isOnGround && !isLookUp)
			LookUp();
		else if (!input.lookUp && isLookUp)
			LookMiddle();
		if (input.lookDown && !isCrouching && isOnGround && !isLookDown)
			LookDown();
		else if (!input.lookDown && isLookDown)
			LookMiddle();
		...
		if (direction == 1)
		{
			...
			//FOVController.DisabledLeft = true;
			//FOVController.DisabledRight = false;
		}
		if (direction == -1)
		{
			animator.SetBool("IsLeft", true);
			if (!isLookUp && !isLookDown)
			{
				FOV.FOVangleSet = 240f;
			}
			//FOVController.DisabledLeft = false;
			//FOVController.DisabledRight = true;
		}
		if (isCrouching)
		...
		if (isOnGround && !isCrouching && !isLookUp && !isLookDown && Mathf.Abs(currentSpeed) > 0) IsRunning = true;
		if (isOnGround)
	...
	{
		isLookUp = true;
		FOV.FOVangleSet = 150f;
	}
	void LookDown()
	{
		isLookDown = true;
		FOV.FOVangleSet = 330f;
	}
